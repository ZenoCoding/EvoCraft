...
public class AttackAbility extends Ability<EntityDamageByEntityEvent> {
    ...
    public static void vertexAbility(EntityDamageByEntityEvent event, Player p, ItemStack item) {
        ...
        if (!stackValues.isEmpty() && !idValues.isEmpty() && idValues.get(0).asInt() == event.getEntity().getEntityId()) {
            ...
            p.setMetadata("vertex_stack", new FixedMetadataValue(EvoCraft.getPlugin(), stacks + 1));
        } else {
            p.setMetadata("vertex_stack", new FixedMetadataValue(EvoCraft.getPlugin(), 1));
        }
        p.setMetadata("vertex_hit_id", new FixedMetadataValue(EvoCraft.getPlugin(), event.getEntity().getEntityId()));
        ...
        if (taskIdValues.isEmpty() || !Bukkit.getScheduler().isQueued(taskIdValues.get(0).asInt())) {
            BukkitTask vertexDisplayTask = new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (stacks <= 0) {
                        cancel();
                        p.setMetadata("vertex_stack", new FixedMetadataValue(EvoCraft.getPlugin(), 0));
                    }
            ...
            }.runTaskTimer(EvoCraft.getPlugin(), 0, 7);


            p.setMetadata("vertex_task_id", new FixedMetadataValue(EvoCraft.getPlugin(), vertexDisplayTask.getTaskId()));
        }
    }
